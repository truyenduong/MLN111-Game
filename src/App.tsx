...
function Game({ onFinish }: { onFinish: (score: number, records: AnswerRecord[], elapsed: number) => void }) {
  ...
  const goBack = () => {
    if (index === 0 || stageCleared) return
    const previousQuestionId = questions[index - 1].id
    const remainingRecords = records.filter(record => record.questionId !== previousQuestionId)
    setRecords(remainingRecords)
    setScore(remainingRecords.filter(record => record.isCorrect).length * 10)
    setIndex(index - 1)
    setSelected(null)
  }
